...
//$rates = mysqli_fetch_row($ratesrows);
   ...
   // $sql = " INSERT INTO `payment` ( `f_no` ,'f_name',  `r_dt` ,  `r_dt`,'name' ) VALUES(  '{$f_no}' , '{$farmer['f_ac']}', '{$mysqldate}' ,  '{$farmer['f_ac']}' ,  '{$authority}'  ) ";
   // mysqli_query($conn,$sql) or die(mysqli_error($conn));
    $insertcmd = mysqli_query($conn,$updatesql) ;
    
    ?>
    <div id="printable">
        <table id="receipt"  >
            <thead style="margin-bottom: 20px">
            <th colspan="2"  ><h1>Wakulima Payment Receipt</h1></th>
            </thead>
            <tbody>
