...
<form action='' method='POST'> 
    ...
    <div class="control-group">     
        ...
        <div class="controls">  
            <?php    
              $result = mysqli_query($conn,"SELECT * FROM `employees`limit 1") or trigger_error(mysqli_error($conn)); 
...
<!--            <input class="input-xlarge" type="text" name='e_role' value='<?php echo stripslashes($row['e_role']) ?>' /> -->
            <select class="input-xlarge" name='e_role'> 
               ...
               <option  <?php echo  $selected=(stripslashes($row['e_role'])=='Farmer'?"selected='selected'":""); echo $selected ?>>Farmer</option> 
